<?php
/**
 * 🚀 MariaDB ECS VPC Initialization
 * Initialize the new MariaDB RDS instance in ECS VPC with schema and data
 */

echo "🔧 MariaDB ECS VPC Initialization\n";
echo "====================================\n\n";
...
do {
    $attempt++;
    echo "Attempt $attempt/$max_attempts: Checking MariaDB status...\n";
    
    $output = shell_exec('aws rds describe-db-instances --db-instance-identifier purrr-mariadb-ecs --query "DBInstances[0].[DBInstanceStatus,Endpoint.Address]" --output json 2>/dev/null');
    ...
    if ($output) {
        ...
        if ($result && count($result) == 2) {
            ...
            if ($status === 'available' && $endpoint) {
                ...
                echo "✅ MariaDB is available at: $endpoint\n\n";
                break;
            }
        }
    }
...
if (!$db_endpoint) {
    echo "❌ MariaDB instance not available after " . ($max_attempts * 30) . " seconds\n";
    echo "Please check the AWS console and run this script again later.\n";
    exit(1);
}
...
try {
    echo "📡 Connecting to MariaDB ECS instance...\n";
    echo "Host: $db_host\n";
    echo "Port: $db_port\n";
    echo "User: $db_user\n\n";
    ...
    echo "✅ Connected to MariaDB ECS successfully!\n\n";
    ...
    // Create admin user with SAME password as before
    ...
    // Create test user with SAME password as before
    ...
    echo "\n🎉 MariaDB ECS initialization completed successfully!\n";
